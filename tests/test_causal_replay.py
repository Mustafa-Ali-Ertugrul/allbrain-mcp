...
class TestCausalReplay:
    ...
    def test_round_trip_counterfactual(self):
        evts = [
            E(EventType.AGENT_COUNTERFACTUAL_RUN.value, "e1",
    ...
    def test_round_trip_impact(self):
        evts = [
            E(EventType.AGENT_CAUSAL_IMPACT_RECORDED.value, "e1",
              make_impact_payload(agent_id="x", task_type="y", alternative_agent="z", impact_score=-0.2, confidence=0.5, sample_count=5)),
        ]
        f = EventReplayEngine().replay(evts)["final_state"]
    ...
    def test_mixed_with_other_reducers(self):
        ...
        evts = [
            ...
            E(EventType.AGENT_COUNTERFACTUAL_RUN.value, "e2",
              make_counterfactual_payload(agent_id="a", task_type="t", actual_agent="a", alternative_agent="b", actual_outcome=0.5, alternative_outcome=0.8, impact_score=0.3, confidence=0.8, sample_count=5)),
