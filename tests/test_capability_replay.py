    ...
    def test_round_trip(self):
        evts = [
            ...
            E(
                EventType.CAPABILITY_MATCHED.value,
                "2",
                make_matched_payload(agent_id="b", task_type="x", match_score=0.3, match_kind="partial"),
            ),
        ]
    ...
    def test_equality(self):
        evts = [
            E(
                EventType.CAPABILITY_MATCHED.value,
                "1",
                make_matched_payload(agent_id="x", task_type="tt", match_score=0.5, match_kind="partial"),
            )
        ]
        assert (
            EventReplayEngine().replay(evts)["final_state"]["capabilities"]
            == EventReplayEngine().replay(evts)["final_state"]["capabilities"]
        )
