...
class TestDecisionRegression:
    ...
    def test_causal_determinism_kept(self):
        ctx = DecisionContext(agent_id="a", task_type="t", contract=make_contract(causal=True),
    ...
    def test_all_modes_reachable(self):
        ...
        for flags in [{"debug": True, "fusion": True}, {"fusion": True}, {"causal": True}, {"dynamics": True}, {}]:
            ctx = DecisionContext(agent_id="a", task_type="t", contract=make_contract(**flags),
                                  fusion={"capability": 0.8, "learning": 0.7, "dynamics": 0.5, "causal": 0.6} if flags.get("fusion") else None,
