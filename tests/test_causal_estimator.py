...
def _build_events(a_scores, b_scores, task_type="bug_fix"):
    ...
    for i, s in enumerate(a_scores):
        evts.append(E(EventType.TASK_COMPLETED.value, f"a{i}", {"agent_id": "a", "task_type": task_type, "outcome_score": s, "runtime_score": s}))
    for i, s in enumerate(b_scores):
        evts.append(E(EventType.TASK_COMPLETED.value, f"b{i}", {"agent_id": "b", "task_type": task_type, "outcome_score": s, "runtime_score": s}))
    return evts
