...
class TestAttributionReplay:
    ...
    def test_round_trip_update(self):
        evts = [
            E(EventType.SIGNAL_ATTRIBUTION_UPDATED.value, "e1",
              make_attribution_update_payload(signal="causal", ema_reward=0.72, count=5)),
            E(EventType.SIGNAL_CREDIT_ASSIGNED.value, "e2",
    ...
    def test_round_trip_importance(self):
        evts = [
            E(EventType.SIGNAL_IMPORTANCE_CHANGED.value, "e1",
    ...
    def test_mixed_with_meta_policy(self):
        ...
        evts = [
            ...
            E(EventType.SIGNAL_ATTRIBUTION_UPDATED.value, "e3",
    ...
    def test_no_duplicate_emission(self):
        evts = [
            E(EventType.SIGNAL_CREDIT_ASSIGNED.value, "dup", make_credit_payload(decision_id="d", signal="s", contribution=0.5, confidence=0.5)),
            E(EventType.SIGNAL_CREDIT_ASSIGNED.value, "dup", make_credit_payload(decision_id="d", signal="s", contribution=0.9, confidence=0.5)),
        ]
        f = EventReplayEngine().replay(evts)["final_state"]
