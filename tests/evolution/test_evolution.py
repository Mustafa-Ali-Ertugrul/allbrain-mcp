...
def seed_learning_history(context) -> None:
    assert save_event_impl(
        ...
        payload={"collaboration_id": "c1", "team_name": "research", "objective": "Analyze", "task_id": "t1"},
        agent_id="supervisor",
    ).ok
    assert save_event_impl(
        context,
    ...
    assert save_event_impl(
        ...
        payload={"consensus_id": "s1", "agent_id": "planner", "vote": "accept", "weight": 0.7},
        agent_id="planner",
    ).ok
    assert save_event_impl(
        context,
        type=EventType.VOTE_CAST.value,
        payload={"consensus_id": "s1", "agent_id": "reviewer", "vote": "accept", "weight": 0.3},
        agent_id="reviewer",
    ).ok
    assert save_event_impl(
        context,
        type=EventType.CONSENSUS_REACHED.value,
        payload={"consensus_id": "s1", "mode": "weighted", "decision": "approve"},
        agent_id="supervisor",
    ).ok
    assert save_event_impl(
        context,
        type=EventType.SUPERVISOR_INTERVENTION.value,
        payload={"supervisor_id": "supervisor", "task_id": "t1", "action": "approve_completion"},
        agent_id="supervisor",
    ).ok
    assert save_event_impl(
        context,
        type=EventType.COLLABORATION_COMPLETED.value,
        payload={"collaboration_id": "c1", "team_name": "research", "outcome": "success"},
        agent_id="supervisor",
    ).ok
...
def test_learning_manager_recommendation_and_policy_feedback(tmp_path) -> None:
    ...
    assert (
        RecommendationEngine().generate(
            kind="consensus", subject="weighted", confidence=2, evidence=[], source_metrics={}
        )["confidence"]
        == 1.0
    )
...
def test_learning_events_replay_graph_memory_and_observability(tmp_path) -> None:
    ...
    assert save_event_impl(
        context,
        type=EventType.ORGANIZATIONAL_PATTERN_DISCOVERED.value,
        payload={
            "pattern_id": "pat1",
            "kind": "successful_team_pattern",
            "summary": "research team works",
            "confidence": 0.9,
        },
    ).ok
    assert save_event_impl(
        context,
        type=EventType.RECOMMENDATION_GENERATED.value,
        payload={
            "recommendation_id": "rec1",
            "kind": "team",
            "subject": "research",
            "confidence": 0.9,
            "source_metrics": {"success_rate": 1.0},
            "supporting_evidence": ["pat1"],
        },
    ).ok
    assert save_event_impl(
        context, type=EventType.RECOMMENDATION_APPLIED.value, payload={"recommendation_id": "rec1"}
    ).ok
    assert save_event_impl(
        context,
        type=EventType.POLICY_UPDATE_PROPOSED.value,
        payload={"policy_update_id": "pu1", "recommendation_id": "rec1"},
    ).ok
    assert save_event_impl(
        context,
        type=EventType.POLICY_UPDATE_APPROVED.value,
        payload={"policy_update_id": "pu1", "recommendation_id": "rec1"},
    ).ok
