...
def _make_events():
    ...
    for i, (aid, score) in enumerate(
        [("a", 0.8), ("a", 0.85), ("a", 0.9), ("a", 0.82), ("a", 0.88),
         ("b", 0.6), ("b", 0.55), ("b", 0.5), ("b", 0.58), ("b", 0.52)]
    ):
        evts.append(E(EventType.TASK_COMPLETED.value, f"t{i}", {
            "agent_id": aid, "task_type": "bug_fix",
            "outcome_score": score, "runtime_score": score,
        }))
    return evts
...
class TestCounterfactualSimulation:
    def test_same_agent_zero_diff(self):
        ...
        r = simulate_intervention(
            agent_id="a", task_type="bug_fix",
            actual_agent="a", alternative_agent="a",
            events=evts,
        )
    ...
    def test_swap_changes_outcome(self):
        ...
        r = simulate_intervention(
            agent_id="a", task_type="bug_fix",
            actual_agent="a", alternative_agent="b",
            events=evts,
        )
    ...
    def test_multi_intervention_isolation(self):
        evts = _make_events()
        r1 = simulate_intervention(agent_id="a", task_type="bug_fix", actual_agent="a", alternative_agent="b", events=evts)
        r2 = simulate_intervention(agent_id="b", task_type="bug_fix", actual_agent="b", alternative_agent="a", events=evts)
    ...
    def test_no_side_effects(self):
        evts1 = _make_events()
        evts2 = _make_events()
        r1 = simulate_intervention(agent_id="a", task_type="bug_fix", actual_agent="a", alternative_agent="b", events=evts1)
        r2 = simulate_intervention(agent_id="a", task_type="bug_fix", actual_agent="a", alternative_agent="b", events=evts2)
        assert r1.impact_score == r2.impact_score

    def test_event_integrity(self):
        evts = _make_events()
        r = simulate_intervention(agent_id="a", task_type="bug_fix", actual_agent="a", alternative_agent="b", events=evts)
    ...
    def test_payloads(self):
        cf = make_counterfactual_payload(
            agent_id="a", task_type="t", actual_agent="a", alternative_agent="b",
            actual_outcome=0.5, alternative_outcome=0.8,
            impact_score=0.3, confidence=0.8, sample_count=5,
        )
        assert cf["impact_score"] == 0.3
        validate_counterfactual(cf)

        imp = make_impact_payload(
            agent_id="a", task_type="t", alternative_agent="b",
            impact_score=0.3, confidence=0.8, sample_count=5,
        )
    ...
    def test_causal_scoring(self):
        s = causal_selection_score(
            ...
            impact_score=0.3, causal_confidence=0.8,
        )
    ...
    def test_frozen_states(self):
        cf = CounterfactualResult(agent_id="a", task_type="t", actual_agent="a", alternative_agent="b", actual_outcome=0.5, alternative_outcome=0.8, impact_score=0.3, confidence=0.8, sample_count=5, direction="positive", analysis_id="x")
        assert cf.agent_id == "a"
        ci = CausalImpact(agent_id="a", task_type="t", alternative_agent="b", impact_score=0.3, confidence=0.8, sample_count=5, analysis_id="x")
