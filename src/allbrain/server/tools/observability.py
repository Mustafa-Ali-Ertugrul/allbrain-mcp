...
def get_observability_dashboard_impl(context: BrainContext, **kwargs: Any) -> ToolResult:
    ...
    audit_tool_call(
        context, tool_name="get_observability_dashboard", tool_args={"limit": data.limit}, session_id=bound_session_id
    )
...
def get_workflow_trace_impl(context: BrainContext, **kwargs: Any) -> ToolResult:
    ...
    result = ObservabilityAPI().workflow_trace(
        events, workflow_id=kwargs.get("workflow_id"), task_id=kwargs.get("task_id")
    )
    audit_tool_call(
        context,
...
def get_system_metrics_impl(context: BrainContext, **kwargs: Any) -> ToolResult:
    ...
    audit_tool_call(context, tool_name="get_system_metrics", tool_args={"limit": limit}, session_id=bound_session_id)
    return ToolResult(ok=True, data=result)
...
def get_reliability_status_impl(context: BrainContext, **kwargs: Any) -> ToolResult:
    ...
    audit_tool_call(
        context, tool_name="get_reliability_status", tool_args={"limit": limit}, session_id=bound_session_id
    )
    return ToolResult(ok=True, data=result)
...
def get_workflow_graph_impl(context: BrainContext, **kwargs: Any) -> ToolResult:
    ...
    bound_session_id = bind_session_id(context, None)
    events = context.repository.list_events(project_path=context.project_path, limit=limit)
    result = ObservabilityAPI().graph(events, workflow_id=kwargs.get("workflow_id"), task_id=kwargs.get("task_id"))
    audit_tool_call(
        context,
...
def compare_agents_impl(context: BrainContext, **kwargs: Any) -> ToolResult:
    ...
    audit_tool_call(context, tool_name="compare_agents", tool_args={"limit": data.limit}, session_id=bound_session_id)
...
def register_tools(mcp, context: BrainContext) -> None:
    ...
    def get_observability_dashboard(limit: int = 5000) -> dict[str, Any]:
        """Observability dashboard: agents, workflows, system overview (read-only).
