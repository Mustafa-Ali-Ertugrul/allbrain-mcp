...
def make_started_payload(
    *, agent_id: str, task_id: str, tool_name: str,
    template_version: int = TELEMETRY_TEMPLATE_VERSION,
) -> dict:
    p = {
        "agent_id": str(agent_id), "task_id": str(task_id),
        "tool_name": str(tool_name), "template_version": int(template_version),
    }
...
def make_completed_payload(
    *, agent_id: str, task_id: str, tool_name: str,
    duration_ms: float, success: bool, retry_count: float,
    template_version: int = TELEMETRY_TEMPLATE_VERSION,
) -> dict:
    p = {
        "agent_id": str(agent_id), "task_id": str(task_id),
        "tool_name": str(tool_name), "duration_ms": float(duration_ms),
        "success": bool(success), "retry_count": float(retry_count),
...
def make_runtime_updated_payload(
    *, agent_id: str, mean_duration_ms: float,
    success_rate: float, mean_retry_count: float, runtime_score_val: float,
    template_version: int = TELEMETRY_TEMPLATE_VERSION,
) -> dict:
    p = {
