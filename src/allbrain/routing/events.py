...
def make_req_payload(*, task_id: str, task_type: str, context_key: str,
                     tv: int = ROUTING_TEMPLATE_VERSION) -> dict:
    p = {"task_id": str(task_id), "task_type": str(task_type),
         "context_key": str(context_key), "template_version": int(tv)}
    validate_req(p)
    return p


def make_scored_payload(*, agent_id: str, task_type: str, selection_score: float,
                        reputation: float, runtime_score: float, calibrated_trust: float,
                        rank: int = 0, tv: int = ROUTING_TEMPLATE_VERSION) -> dict:
    p = {"agent_id": str(agent_id), "task_type": str(task_type),
         "selection_score": float(selection_score), "reputation": float(reputation),
         "runtime_score": float(runtime_score), "calibrated_trust": float(calibrated_trust),
         "rank": int(rank), "template_version": int(tv)}
    validate_scored(p)
    return p


def make_selected_payload(*, task_id: str, task_type: str, agent_id: str,
                          selection_score: float, tv: int = ROUTING_TEMPLATE_VERSION) -> dict:
    p = {"task_id": str(task_id), "task_type": str(task_type),
         "agent_id": str(agent_id), "selection_score": float(selection_score),
