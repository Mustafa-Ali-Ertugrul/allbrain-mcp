    ...
    def build(self, events: list[EventRead]) -> dict[str, Any]:
        ...
        for event in canonical_event_sort(events):
            ...
            if event.type == EventType.GOVERNANCE_REVIEW_INITIATED.value and isinstance(review_id, str):
                state["reviews"][review_id] = {"review_id": review_id, "status": "initiated", "events": [event.id], **payload}
            elif event.type == EventType.GOVERNANCE_ALIGNMENT_EVALUATED.value and isinstance(review_id, str):
                review = state["reviews"].setdefault(review_id, {"review_id": review_id, "status": "initiated", "events": []})
                ...
                if isinstance(review_id, str):
                    ...
                    review["events"].append(event.id)
                state["current_autonomy_level"] = max(state["current_autonomy_level"], int(payload.get("autonomy_level_allowed", 0) or 0))
        ...
        state["current_autonomy_level"] = AutonomyBoundaryController().next_allowed_level_from_events(successful_post_checks, state["current_autonomy_level"])
        return state
