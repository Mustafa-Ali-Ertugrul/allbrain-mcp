...
class ArbitrationBridge:
    def arbitrate(self, governance: dict[str, Any], economic: dict[str, Any], execution_plan: dict[str, Any]) -> dict[str, Any]:
        conflicts: list[str] = []
        ...
        if economic["decision"] in {"delay", "request_research"} and execution_plan["strategy"] != "research_first_execution":
            conflicts.append("economic_uncertainty_vs_execution")
        if governance_decision == "approve_with_constraints" and execution_plan["strategy"] == "adaptive_hybrid_execution":
