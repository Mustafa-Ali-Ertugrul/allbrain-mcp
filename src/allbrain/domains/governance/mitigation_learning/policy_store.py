...
class PolicyStore:
    ...
    def update_if_needed(
        ...
        if current is not None:
            ...
            if all_keys:
                max_delta = max(abs(new_success_rates.get(k, 0.0) - old_rates.get(k, 0.0)) for k in all_keys)
        ...
        policy = PolicyVersion(
            ...
            strategy_preferences={s.strategy: s.success_rate * max(0.0, s.avg_effectiveness) for s in relevant},
            disabled_strategies=frozenset(s.strategy for s in relevant if s.disabled),
            urgency_multipliers={s.strategy: _clamp(1.0 + s.avg_effectiveness) for s in relevant},
            stats_snapshot={
                "success_rates": new_success_rates,
                "avg_effectiveness": {s.strategy: s.avg_effectiveness for s in relevant},
