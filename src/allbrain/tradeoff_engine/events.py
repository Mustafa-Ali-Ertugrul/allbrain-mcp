...
def make_tradeoff_analyzed_payload(*, fault_type: str, frontier_size: int, dominated_count: int,
    tv: int = TRADEOFF_ENGINE_TEMPLATE_VERSION,
) -> dict[str, Any]:
    p = {"fault_type": fault_type, "frontier_size": frontier_size, "dominated_count": dominated_count, "template_version": tv}
    validate_tradeoff_analyzed(p)
    return p

def make_utility_computed_payload(*, policy_id: str, fault_type: str, utility: float, safety_pass: bool,
    tv: int = TRADEOFF_ENGINE_TEMPLATE_VERSION,
) -> dict[str, Any]:
    p = {"policy_id": policy_id, "fault_type": fault_type, "utility": round(utility, 4),
