...
class WorkflowGraphBuilder:
    def build(self, events: list[EventRead]) -> dict[str, Any]:
        ...
        for event in canonical_event_sort(events):
            ...
            if workflow_id:
                nodes.setdefault(f"workflow:{workflow_id}", {"id": f"workflow:{workflow_id}", "type": "workflow", "workflow_id": workflow_id})
            if task_id:
                task_node = nodes.setdefault(f"task:{task_id}", {"id": f"task:{task_id}", "type": "task", "task_id": task_id})
                if workflow_id:
            ...
            if event.type == EventType.TASK_DEPENDENCY_ADDED.value and task_id:
                ...
                nodes[decision_id] = {"id": decision_id, "type": "selection_decision", "task_id": task_id, "agent_id": event.payload.get("agent_id")}
                _add_edge(edges, decision_id, f"task:{task_id}", "flow")
            elif event.type in {EventType.AGENT_EXECUTION_STARTED.value, EventType.AGENT_EXECUTION_COMPLETED.value, EventType.AGENT_EXECUTION_FAILED.value}:
...
def _add_collaboration_graph(nodes: dict[str, dict[str, Any]], edges: list[dict[str, str]], event: EventRead) -> None:
    ...
    if event.type.startswith("collaboration_") and isinstance(collaboration_id, str):
        ...
        nodes[cid] = {"id": cid, "type": "collaboration", "collaboration_id": collaboration_id, "status": event.type.replace("collaboration_", "")}
    ...
    if event.type.startswith("delegation_") and isinstance(delegation_id, str):
        ...
        nodes[did] = {"id": did, "type": "delegation", "delegation_id": delegation_id, "status": event.type.replace("delegation_", "")}
    ...
    if event.type.startswith("negotiation_") and isinstance(negotiation_id, str):
        ...
        nodes[nid] = {"id": nid, "type": "negotiation", "negotiation_id": negotiation_id, "status": event.type.replace("negotiation_", "")}
    proposal_id = payload.get("proposal_id")
    if event.type.startswith("proposal_") and isinstance(proposal_id, str):
        pid = f"proposal:{proposal_id}"
        nodes[pid] = {"id": pid, "type": "proposal", "proposal_id": proposal_id, "status": event.type.replace("proposal_", "")}
        agent_id = payload.get("agent_id")
        if isinstance(agent_id, str):
            nodes.setdefault(f"agent:{agent_id}", {"id": f"agent:{agent_id}", "type": "agent", "agent_id": agent_id})
            _add_edge(edges, f"agent:{agent_id}", pid, "proposed_by")
        if isinstance(negotiation_id, str):
            nodes.setdefault(f"negotiation:{negotiation_id}", {"id": f"negotiation:{negotiation_id}", "type": "negotiation", "negotiation_id": negotiation_id})
            _add_edge(edges, pid, f"negotiation:{negotiation_id}", "flow")
    consensus_id = payload.get("consensus_id")
    if event.type in {EventType.VOTE_CAST.value, EventType.CONSENSUS_REACHED.value, EventType.CONSENSUS_FAILED.value} and isinstance(consensus_id, str):
    ...
    if event.type == EventType.SUPERVISOR_INTERVENTION.value:
        ...
        nodes[sid] = {"id": sid, "type": "supervisor_action", "supervisor_id": payload.get("supervisor_id"), "action": payload.get("action")}
        supervisor_id = payload.get("supervisor_id")
        if isinstance(supervisor_id, str):
            nodes.setdefault(f"agent:{supervisor_id}", {"id": f"agent:{supervisor_id}", "type": "agent", "agent_id": supervisor_id})
...
def _add_learning_graph(nodes: dict[str, dict[str, Any]], edges: list[dict[str, str]], event: EventRead) -> None:
    ...
    if event.type.startswith("learning_cycle_") and isinstance(cycle_id, str):
        ...
        nodes[node_id] = {"id": node_id, "type": "learning_event", "cycle_id": cycle_id, "status": event.type.replace("learning_cycle_", "")}
    ...
    if event.type.startswith("recommendation_") and isinstance(recommendation_id, str):
        ...
        nodes[node_id] = {"id": node_id, "type": "recommendation", "recommendation_id": recommendation_id, "status": event.type.replace("recommendation_", ""), "kind": payload.get("kind")}
    ...
    if event.type.startswith("policy_update_") and isinstance(policy_update_id, str):
        ...
        nodes[node_id] = {"id": node_id, "type": "policy_update", "policy_update_id": policy_update_id, "status": event.type.replace("policy_update_", "")}
        recommendation_id = payload.get("recommendation_id")
        ...
        nodes.setdefault("policy:agent_selection", {"id": "policy:agent_selection", "type": "policy", "policy_id": "agent_selection"})
...
def _add_governance_graph(nodes: dict[str, dict[str, Any]], edges: list[dict[str, str]], event: EventRead) -> None:
    ...
    if event.type == EventType.GOVERNANCE_REVIEW_INITIATED.value:
        nodes[review_node].update({"status": "initiated", "trigger_source": payload.get("trigger_source"), "system_area": payload.get("system_area")})
        ...
        nodes[node_id] = {"id": node_id, "type": "alignment_report", "review_id": review_id, "alignment_score": payload.get("alignment_score")}
        ...
        nodes[node_id] = {"id": node_id, "type": "system_trajectory", "review_id": review_id, "trajectory_score": payload.get("trajectory_score")}
        ...
        nodes[node_id] = {"id": node_id, "type": "autonomy_decision", "review_id": review_id, "autonomy_level_allowed": payload.get("autonomy_level_allowed")}
        ...
        nodes[node_id] = {"id": node_id, "type": "governance_decision", "review_id": review_id, "decision_id": decision_id, "decision": payload.get("decision")}
        ...
        nodes[node_id] = {"id": node_id, "type": "governance_constraint", "review_id": review_id, "constraints": payload.get("constraints", [])}
        nodes.setdefault("policy:autonomous_governance", {"id": "policy:autonomous_governance", "type": "policy", "policy_id": "autonomous_governance"})
        ...
        nodes[node_id] = {"id": node_id, "type": "governance_post_check", "review_id": review_id, "status": payload.get("status")}
...
def _add_runtime_core_graph(nodes: dict[str, dict[str, Any]], edges: list[dict[str, str]], event: EventRead) -> None:
    ...
    if event.type == EventType.PIPELINE_RUN_STARTED.value:
        ...
        nodes[state_node] = {"id": state_node, "type": "pipeline_state", "run_id": run_id, "status": payload.get("new_status")}
        ...
        objective_id = objective.get("objective_id") or objective.get("task_id") or run_id if isinstance(objective, dict) else run_id
        ...
        nodes[node_id] = {"id": node_id, "type": "economic_evaluation", "run_id": run_id, "decision": payload.get("decision")}
        ...
        nodes[node_id] = {"id": node_id, "type": "runtime_arbitration", "run_id": run_id, "action": payload.get("action")}
        ...
        nodes[node_id] = {"id": node_id, "type": "prediction_error", "run_id": run_id, "error_delta": payload.get("error_delta")}
...
def _event_backed_node_id(event: EventRead) -> str | None:
    ...
    for key, prefix in [("recommendation_id", "recommendation"), ("policy_update_id", "policy_update"), ("pattern_id", "optimization"), ("cycle_id", "learning_event")]:
    ...
    for key, prefix in [("proposal_id", "proposal"), ("delegation_id", "delegation"), ("negotiation_id", "negotiation"), ("collaboration_id", "collaboration"), ("consensus_id", "consensus")]:
        value = event.payload.get(key)
        if isinstance(value, str) and value:
            return f"{prefix}:{value}"
    task_id = _task_id(event)
