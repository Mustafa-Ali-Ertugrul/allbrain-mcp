...
BUDGET_KEYS: frozenset[str] = frozenset(
    {"total_budget", "unused_budget", "allocated_total"}
)

REALLOC_KEYS: frozenset[str] = frozenset(
    {"signal", "delta_allocation", "new_allocation"}
)
...
def make_attention_payload(
    *, signal: str, importance: float, cost: float, allocation: float,
    tv: int = ATTENTION_TEMPLATE_VERSION,
) -> dict:
    p = {"signal": signal, "importance": float(importance), "cost": float(cost),
         "allocation": float(allocation), "template_version": tv}
    validate_attention(p)
    return p


def make_budget_payload(
    *, total_budget: float, unused_budget: float, allocated_total: float,
    tv: int = ATTENTION_TEMPLATE_VERSION,
) -> dict:
    p = {"total_budget": float(total_budget), "unused_budget": float(unused_budget),
         "allocated_total": float(allocated_total), "template_version": tv}
    validate_budget(p)
    return p


def make_reallocation_payload(
    *, signal: str, delta_allocation: float, new_allocation: float,
    tv: int = ATTENTION_TEMPLATE_VERSION,
) -> dict:
    p = {"signal": signal, "delta_allocation": float(delta_allocation),
         "new_allocation": float(new_allocation), "template_version": tv}
