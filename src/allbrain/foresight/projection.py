...
class ForesightProjection:
    def build(self, events: list[EventRead]) -> dict[str, Any]:
        ...
        for event in sorted(events, key=lambda item: (item.created_at, item.id)):
            if event.type == EventType.FORESIGHT_GENERATED.value:
                ...
                if isinstance(aid, str) and aid not in seen_ids:
                    ...
                    seen_ids.add(aid)
            elif event.type == EventType.FORESIGHT_EVALUATED.value:
                evaluated.append(event.payload)
            elif event.type == EventType.FORESIGHT_RECOMMENDED.value:
