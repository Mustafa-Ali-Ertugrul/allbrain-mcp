...
class LearningStateBuilder:
    def build(self, events: list[EventRead]) -> dict[str, Any]:
        ...
        for event in canonical_event_sort(events):
            ...
            if event.type in {EventType.RECOMMENDATION_GENERATED.value, EventType.RECOMMENDATION_APPLIED.value, EventType.RECOMMENDATION_REJECTED.value} and isinstance(recommendation_id, str):
                recommendation = state["recommendations"].setdefault(recommendation_id, {"recommendation_id": recommendation_id, "status": "generated"})
            ...
            if event.type in {EventType.POLICY_UPDATE_PROPOSED.value, EventType.POLICY_UPDATE_APPROVED.value, EventType.POLICY_UPDATE_REJECTED.value} and isinstance(policy_update_id, str):
                update = state["policy_updates"].setdefault(policy_update_id, {"policy_update_id": policy_update_id, "status": "proposed"})
